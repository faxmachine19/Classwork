  ...
  (match p
    [(Prog ds e)
     (prog (externs)
           ...
           (Mov 'r9 0) ;; If e empty, set r9 for 0 returned args
           ;; ^^ if we don't, convert-return tries to create a vector
           ;; of size equals whatever r9 had, could easily be bigger than heap
...
(define (compile-e e c t?)
  (match e
    ...
    [(Vals es)          (compile-values es c)] 
    [(LetVal xs e1 e2)  (compile-letval xs e1 e2 c t?)]))


;; Final project functions included here to make scrolling easier
(define (convert-return)
  (let ((loop (gensym 'loop))
        (end (gensym 'end))
        (one (gensym 'one)))
    (seq (Sal 'r9 int-shift) ;; Turn arg count into loot int
         (Push 'r9) ;; Push return arg count to stack to make return vector,
         (Push 'r9) ;; Push a second copy to not clobber
         (compile-op2 'make-vector) ;; populate with whatever rax is since we overwrite
         (Pop 'r9) ;; Restore copy of arg count to r9
         (Cmp 'r9 (value->bits 1)) ;; If single arg, then rax had return value,
         (Je end) ;; so we have length one vector of all return values
         (Mov 'r11 'r9) ;; Load max length to register vector-set! doesn't use
         (Sub 'r11 (value->bits 1)) ;; and sub 1 so we can iterate backwards through indices
         (Label loop) ;; Now, want to fill that vector with return values
         (Cmp 'r11 0) ;; If index to change is no longer positive,
         (Jl end) ;; all indices changed, so jump to end
         ;; Stack is e1|...|en
         ;; We setup e1|...|en|vec|vec|ind and rax=en,
         ;; vector-set! clears ...|vec|ind,
         (Push 'rax) ;; Push vector pointer
         (Push 'rax) ;; and a second copy
         (Mov 'rax (Offset 'rsp 16)) ;; Load rax with top return value on stack
         (Push 'r11) ;; Push index to modify on stack (loot int)
         (compile-op3 'vector-set!)
         (Pop 'rax) ;; stack ends ...|en-1|en|vec, 
         (Add 'rsp 8) ;; want ...|en-1|vec
         (Sub 'r11 (value->bits 1)) ;; Subtract 1 from position in loot ints
         (Jmp loop)
         (Label end)
         (Xor 'rax type-vect) ;; Clear vector tag for C to use
       )))

(define (compile-values es c)
  (let ((end (gensym 'end)))
    (seq (compile-es es c) ;; Compile-es modified to check arity==1 for each es
         (Mov 'r9 (length es))
         (Cmp 'r9 1) ;; If only one element,
         (Jne end) 
         (Pop 'rax) ;; then pop from stack to rax (like any other expression)
         (Label end))))

;; Is t? needed?
(define (compile-letval xs e1 e2 c t?)
  (let ((end (gensym 'end)))
    (seq (Mov 'r10 (length xs)) ;; Store length of expected vals
         (compile-e e1 c #f) ;; Compile e1 (if values, then r9 has given count)
         (Cmp 'r9 'r10) ;; If expected != given vals, error
         (Jne 'raise_error_align)
         (Cmp 'r9 1)
         (Jne end) ;; If only one argument,
         (Push 'rax) ;; push it to stack
         (Label end)
         (compile-e e2 (append (reverse xs) c) t?)
         (Add 'rsp (* 8 (length xs))) ;; Pop all args off of stack since ^^ doesn't consume
         )))
...
(define (compile-prim1 p e c)
  (seq (compile-e e c #f)
       (Cmp 'r9 1) ;; Final project change; arity check
       (Jne 'raise_error_align) 
       (compile-op1 p)
       (Mov 'r9 1) ;; Final project change
       ))
...
(define (compile-if e1 e2 e3 c t?)
  (let ((l1 (gensym 'if))
        (l2 (gensym 'if)))
    (seq (compile-e e1 c #f)
         ;; Only e1 is required to return 1 value
         (Cmp 'r9 1) ;; Final project change; arity check
         (Jne 'raise_error_align) 
         (Cmp rax val-false)
         (Je l1)
...
(define (compile-lam f xs e c) 
  (let ((fvs (fv (Lam f xs e))))
    (seq (Lea rax (symbol->label f))
         ...
         (Mov 'r9 1) ;; Final project; return arity
