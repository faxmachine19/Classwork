...
(define (test-runner run)
  ;; Given code for testing just values
  (define (check-equal?* prog expected-list)
    (check-equal? (call-with-values
                ...
                expected-list))
  ;; Basic tests
  (check-equal? (run '(let-values ([(x) (values 1)]) (add1 x)))
                2)
  (check-equal? (run '(let-values ([(x y) (values 1 2)]) (+ x y)))
                3)
  (check-equal? (run '(let-values ([(x y) (values 1 2)]) y))
                2)
  (check-equal? (run '(let-values ([(x) (values 3)]) 1))
                1)
  (check-equal?* '[(values 1)] '(1))
  (check-equal?* '[(values 1 2)] '(1 2))
  (check-equal?* '[(values)] '())
  ;; More complex expressions inside values
  (check-equal? (run '(let-values ([(x) (values (add1 3))]) x))
                4)
  (check-equal? (run '(let-values ([(x y) (values (add1 4) (+ 3 6))]) (+ x y)))
                14)
  ;; Edge cases
  (check-equal? (run '(let-values ([() (values)]) 1))
                1)
  ;; Arity mismatches
  (check-equal? (run '(let-values ([(x) (values 1 2)]) 1))
                'err)
  (check-equal? (run '(let-values ([(x y) (values 1)]) 1))
                'err)
  (check-equal? (run '(add1 (values 1 2)))
                'err)
  (check-equal? (run '(add1 (values)))
                'err)
  (check-equal? (run '(+ (values 1 2) 1))
                'err)
  (check-equal? (run '(values (values 1 2) 3))
                'err)
